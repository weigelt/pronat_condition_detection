...
public class HeuristicCheck {
	...
	public static void checkForIfClause(INode[] nodes, List<Keyword> ifKeywordsInText) {
		...
		for (Keyword keyword : ifKeywordsInText) {
			...
			if (!SyntaxHelper.isNounPhrase(nodes, begin)) {
				setCommandType(nodes, keyword.getKeywordBegin(), nextStmt - 1, CommandType.INDEPENDENT_STATEMENT);
				...
				if (ConditionDetector.compensateSNLP) { // catch IOB-format-failures of the ASR 
					if (SyntaxHelper.isVerbPhrase(nodes, i)) {
						...
						if (verbCounter == 2 && SyntaxHelper.isVerbPhrase(nodes, i - 1) // 2 consecutive verbs ("if .. playing, go")
							...
							nodes[i].setAttributeValue("commandType", CommandType.THEN_STATEMENT);
			...
			if (beginThenClause) {
				...
				setCommandType(nodes, keyword.getKeywordBegin(), endOfVerbBlock - 1, CommandType.IF_STATEMENT);
				continue;
			}
			...
			setCommandType(nodes, keyword.getKeywordBegin(), endOfIfBlock, CommandType.IF_STATEMENT);
	...
	public static void checkForThenClause(INode[] nodes, List<Keyword> thenKeywordsInText) {
		for (Keyword keyword : thenKeywordsInText) {
			...
			if (keyword.isDummy() && keyword.getCmdtype() == null) { // if no then-Keyword is found, start then-heuristic check at first verb
				...
				if (keyword.isInvalid()) {
					setCommandType(nodes, currStmt, nextStmt - 1, CommandType.INDEPENDENT_STATEMENT);
			...
			if (!SyntaxHelper.isVerbPhrase(nodes, currStmt)) {
				...
				if (endOfThenBlock > currStmt) {
					setCommandType(nodes, currStmt, endOfThenBlock, CommandType.THEN_STATEMENT);
				...
				setCommandType(nodes, keyword.getKeywordBegin(), currStmt, CommandType.INDEPENDENT_STATEMENT);
				keyword.setKeyword("DUMMY_KEYWORD");
				...
				if (SyntaxHelper.isNounPhrase(nodes, wordBeforeVerb) && nodes[wordBeforeVerb].getAttributeValue("commandType") == null) {
					...
					if (endOfThenBlock > currStmt) {
						while (nodes[wordBeforeVerb].getAttributeValue("commandType") == null
							...
							nodes[wordBeforeVerb].setAttributeValue("commandType", CommandType.THEN_STATEMENT);
							wordBeforeVerb--;
						}
						setCommandType(nodes, wordBeforeVerb + 1, endOfThenBlock, CommandType.THEN_STATEMENT);
			...
			setCommandType(nodes, keyword.getKeywordBegin(), endOfThenBlock, CommandType.THEN_STATEMENT);
			logger.debug("End of spotted then-clause at position: " + endOfThenBlock + "\n");
		}
	}

	public static void checkForElseClause(INode[] nodes, List<Keyword> elseKeywordsInText) {
				...
				if (!SyntaxHelper.isVerbPhrase(nodes, currStmt)) {
					if (!keyword.isDummy()) {
						...
						if (endOfElseBlock > currStmt) {
							setCommandType(nodes, keyword.getKeywordBegin(), endOfElseBlock, CommandType.ELSE_STATEMENT);
	 ...
	 * @param currStmt is the first node where to check the heuristic. Mostly
	 *        the one after the keyword/first verb
	 * @param nextStmt in the input
	...
	private static void setCommandType(INode[] nodes, int begin, int end, CommandType cmd) {
		String s = "cmdtype: ";
		for (int i = begin; i <= end; i++) {
			nodes[i].setAttributeValue("commandType", cmd);
			s = s.concat(
